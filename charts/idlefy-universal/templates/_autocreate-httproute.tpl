...
spec:
  ...
  rules:
    ...
    - matches:
        ...
        {{- /* HttpRouteMatch.path is optional (a match can be headers/queryParams/method only);
               `.path.type` on a nil path is a template error, so default it once, next to the range. */}}
        {{- $path := $match.path | default dict }}
        - path:
