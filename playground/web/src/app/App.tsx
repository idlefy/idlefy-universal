  ...
  return (
    <div className="app-shell">
      <header>
        ...
        {state.render?.ok && <span className="muted"> · rendered {state.render.manifests.length} objects in {state.render.durationMs} ms</span>}
        <a className="muted" href="../" style={{ marginLeft: 'auto' }}>docs</a>
      </header>
      <div className="split">
        <section className="left">
          <Toolbar release={state.releaseName} ns={state.namespace} onRelease={(v) => dispatch({ type: 'release', v })} onNs={(v) => dispatch({ type: 'ns', v })}
            onPickExample={(id) => { const ex = (examples as { id: string; values: string }[]).find((e) => e.id === id); if (ex) dispatch({ type: 'example', id, text: ex.values }); }}
            valuesText={state.text} chartVersion={chartMeta.version} />
        ...
        <section className="right">
          ...
          <DetailPanel node={selected} onClose={() => dispatch({ type: 'select', id: null })} />
        </section>
      </div>
    </div>
  );
