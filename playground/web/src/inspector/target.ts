...
export function inspectTarget(sel: ResolvedSelection, root: SchemaNode): InspectTarget {
  ...
  if (sel.kind === 'block') {
    ...
    return { kind: 'secondary', owner: sel.path.slice(0, 2), secondary: tail as SecondaryId, path: sel.path, node: null };
  }
  const node = sel.node;
  ...
  if (p.owner && p.path.length === 3) {
    ...
    if (SEC_IDS.has(tail)) return { kind: 'secondary', owner: p.owner, secondary: tail as SecondaryId, path: p.path, node };
  }
